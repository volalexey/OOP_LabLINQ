...
namespace Practice_Linq
{
    public class Program
    {
        ...
        static void Query11(List<FootballGame> games)
        {
            ...
            var selectedGames = games.Where(g => g.Date.Year == 2020)
                                     .Select(g => g.Country)
                                     .Distinct()
                                     .OrderBy(c => c)
                                     .Take(10);
            ...
            foreach (var country in selectedGames) Console.WriteLine(country);
        ...
        static void Query12(List<FootballGame> games)
        {
            ...
            var selectedGames = games.Where(g => g.Date.Year >= 2022)
            ...
            foreach (var item in selectedGames) Console.WriteLine($"{item.Tournament} - {item.Count}");
        ...
        static void Query14(List<FootballGame> games)
        {
            ...
            var selectedGames = games.GroupBy(g => g.Tournament)
                                     .Select(group => new
                                     {
                                        Tournament = group.Key,
                                        AvgGoals = group.Average(g => (double)g.Home_score + g.Away_score)
                                     })
                                     .OrderByDescending(res => res.AvgGoals)
                                     .Take(5);
            ...
            foreach (var item in selectedGames)
            {
                Console.WriteLine($"{item.Tournament} - {item.AvgGoals:F2}");
            }
        ...
        static void Query15(List<FootballGame> games)
        {
            ...
            var selectedGames = games.Select(g => g.Home_team)
                                     .Concat(games.Select(g => g.Away_team))
                                     .GroupBy(t => t)
                                     .Select(group => new { Team = group.Key, Count = group.Count() })
                                     .Where(res => res.Count == 1)
                                     .OrderBy(res => res.Team);


            // Результат
            Console.WriteLine("\n======================== QUERY 15 ========================");

            foreach (var item in selectedGames) Console.WriteLine($"{item.Team} - {item.Count}");
